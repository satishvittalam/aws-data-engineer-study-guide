...
| Amazon OpenSearch Service   | Search & analytics engine                 | Full-text search, log/clickstream analytics, dashboards     |
...
- **Full-text search, log analytics, near real-time dashboards** → OpenSearch Service.
...
- **Cross-region snapshot copy & DR**: Manual/automated snapshots can be copied to another region for disaster recovery; combined with cross-region read replicas, this supports a "restore in another region within X minutes" style exam scenario — distinct from Multi-AZ, which only protects within a region.
...
- **Global Tables**: Fully managed, multi-region, multi-active replication — each region can read and write, with DynamoDB Streams handling propagation and last-writer-wins conflict resolution. Common exam answer for "low-latency access to the same DynamoDB data from users in multiple regions" or "active-active DR for a NoSQL table."
- **On-Demand Backups**: Full backups retained until explicitly deleted, with no performance impact on the table — used for long-term archival/compliance.
- **Point-in-Time Recovery (PITR)**: Continuous backups allowing restore to any second within the last 35 days — the DynamoDB equivalent of RDS automated backups; must be explicitly enabled per table.
- Both backup types restore to a **new table**, not in place — a common exam distractor is assuming an in-place restore.
...
- **Redshift Serverless**: Fully managed, no cluster/node provisioning — automatically scales compute (measured in RPUs) based on workload and pauses when idle, billed per-second of usage. Common exam answer for "unpredictable or infrequent analytics workloads with least operational overhead," distinct from provisioned RA3 clusters which require capacity planning.
- **Distribution styles**:
...
- **Amazon OpenSearch Service**: Managed Elasticsearch/OpenSearch cluster for full-text search, log analytics, and near real-time dashboards (via OpenSearch Dashboards). Commonly used as a Kinesis Data Firehose or CloudWatch Logs destination for operational/clickstream analytics — distinct from Redshift, which is better suited to structured, large-scale historical aggregation rather than search-style queries.
...
3. **Need cross-region DR for RDS with restore capability** → **Cross-region automated/manual snapshot copy** (+ cross-region read replica for lower RTO).
4. **Millisecond-latency NoSQL access at massive scale, unpredictable traffic** → **DynamoDB with On-Demand capacity**.
5. **Need to react to item-level changes in DynamoDB in real time** → **DynamoDB Streams + Lambda**.
6. **Reduce read latency further on DynamoDB without app changes** → **DAX**.
7. **Need active-active DynamoDB access from multiple regions with low latency** → **DynamoDB Global Tables**.
8. **Need long-term, on-demand DynamoDB backup for compliance, restorable to a point in time** → **On-Demand Backup (archival) vs PITR (any second in last 35 days)** — both restore to a new table.
9. **Continuously sync on-prem/RDS database changes into a data lake (S3)** → **DMS with Full Load + CDC**.
10. **Migrating between different DB engines (e.g., Oracle → PostgreSQL)** → **DMS + Schema Conversion Tool (SCT)**.
11. **Query S3 data alongside Redshift tables without loading** → **Redshift Spectrum**.
12. **Speed up repeated complex aggregation queries in Redshift** → **Materialized Views**.
13. **Distribute large fact table efficiently for joins in Redshift** → **KEY distribution style** on the join column.
14. **Small, frequently joined dimension table in Redshift** → **ALL distribution style**.
15. **Unpredictable/infrequent analytics workload, want least operational overhead (no cluster sizing)** → **Redshift Serverless**.
16. **Reduce DB load for frequently-read, rarely-changed data** → **ElastiCache (Redis/Memcached) with lazy loading**.
17. **Need immutable, verifiable audit log of transactions** → **QLDB** (not RDS/DynamoDB).
18. **Graph/relationship-heavy queries (e.g., fraud detection)** → **Neptune**.
19. **Need full-text search or near real-time log/clickstream dashboards** → **OpenSearch Service**.
20. **Rotate DB credentials automatically without app downtime** → **Secrets Manager integration with RDS**.
21. **Access DB using IAM roles instead of static passwords** → **IAM Database Authentication**.
...
- Cross-region snapshot copy = **DR**, not automatic — must be configured
...
- DynamoDB PITR = restore to **any second** in last 35 days | On-Demand Backup = **manual, retained until deleted**
...
- Redshift Serverless = **no cluster management**, pay per RPU-second, auto-pauses when idle
...
- OpenSearch Service = **search & log/clickstream analytics**, not a general OLAP warehouse
